    ...
    <a-form :form="form" layout="vertical">
      <a-row :gutter="10">
        <a-divider orientation="left">
          <span style="font-size: 13px">选择疫苗商家</span>
        </a-divider>
        <a-col :span="12">
          <a-form-item label='疫苗商家'>
            <a-select @change="pharmacyCheck" v-decorator="[
              'pharmacyId',
              { rules: [{ required: true, message: '请输入所属疫苗商家!' }] }
              ]">
      ...
      <a-row :gutter="10">
        <a-divider orientation="left">
          <span style="font-size: 13px">疫苗信息</span>
        </a-divider>
        <a-col :span="24">
          <a-table :columns="columns" :data-source="dataList" :pagination="false">
            ...
            <template slot="typeIdShow" slot-scope="text, record">
              ...
              <span v-if="record.classification == 6">生化疫苗</span>
              <span v-if="record.classification == 7">放射性疫苗</span>
              <span v-if="record.classification == 8">血清</span>
              <span v-if="record.classification == 9">诊断疫苗</span>
            </template>
